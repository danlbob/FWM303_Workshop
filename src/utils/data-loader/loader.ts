...
import { SOURCE_BUCKET_URL } from './loaderimports';
...
async function LoadSampleData(statusCallback: (status: StatusUpdate) => void) {
    ...
    if (SOURCE_BUCKET_URL.includes("cloudfront.net")) {
        ...
        const MAX_PRODUCTS_TO_LOAD = 400;
        let productCount = 0;
        ...
        if ( currCategoryCount !== numCategories ) {
            ...
            for (const cat of categories) {
                ...
                const prodByCat = products.filter((prod) => prod.category === cat.name).slice(0, MAX_PER_CATEGORY);
                for (const prod of prodByCat) {
                    ...
                    if ( errors !== undefined ) {
                        ...
                        productCount += 1;
                        localStatus["productProgress"] = ( productCount / MAX_PRODUCTS_TO_LOAD ) * 100;
                    }
                    statusCallback(localStatus);    
                }
            ...
            if ( productCount > 0 ) {
                ...
                localStatus["productResultText"] = `Success! ${productCount} products are now loaded.`
                statusCallback(localStatus);
            }
        } else {
            localStatus["categoryStatus"] = "success";
